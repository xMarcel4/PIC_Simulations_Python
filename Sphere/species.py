...
class Species:
    ...
    def  advance(self):
        ...
        # Step 1: Update particles and mark those that need to be removed
        for p_idx, part in enumerate(self.particles):
            ...
            if self.world.in_sphere(part.pos):
            ...
            if not self.world.in_bounds(part.pos):
                particles_out_of_bounds += 1
                part.mpw = 0  # Mark the particle as dead by setting its weight to zero
                continue
        
        # Remove particles with zero macroparticle weight (mpw == 0)
        self.particles = [part for part in self.particles if part.mpw > 0]
        
        # Output the number of particles in the sphere, out of bounds, and remaining
        total_particles = len(self.particles)
        # print(f"Particles in sphere: {particles_in_sphere}, Particles out of bounds: {particles_out_of_bounds}, Remaining particles: {total_particles}")
        print(f"\nParticles in sphere: {particles_in_sphere}   Particles out of bounds: {particles_out_of_bounds}")
    ...
    def add_particle(self, pos, vel, mpw):
        ...
        # Ensure vel is a numpy array with float type
